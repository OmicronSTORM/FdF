...
/*   Updated: 2025/03/25 14:57:05 by jowoundi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_project_iso(t_dot *point, int scale)
{
	...
	point->projected_x += WIDTH / 2;
	point->projected_y += HEIGHT / 2;
}
...
void	ft_compute_scale(t_map *info)
{
	...
	if (proj.y_range == 0)
		proj.y_range = 0;
	proj.scale_x = (WIDTH * 0.8) / proj.x_range;
	proj.scale_y = (HEIGHT * 0.8) / proj.y_range;
...
// 	{
// 		info->point[i].projected_x = WIDTH / 2 + (info->point[i].projected_x - proj.center_x) * info->scale;
// 		info->point[i].projected_y = HEIGHT / 2 + (info->point[i].projected_y - proj.center_y) * info->scale;
// 		i++;
// 	}
// }

void	ft_extend(t_map *info)
{
	int	i;
	
	i = 0;
	while (info->x_max < WIDTH - 1 && info->y_max < HEIGHT - 1 
			&& info->x_min > 1 && info->y_min > 1)
	{
		info->x_max++;
		info->x_min--;
		info->y_max++;
